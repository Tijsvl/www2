...
const ContactForm = (props) => {
  ...
  const nameInputHandler = (input) =>
    setNameInput(validateLength('Name', input, 2, 32));
  const subjectInputHandler = (input) =>
    setSubjectInput(validateLength('Subject', input, 0, 32));
  const messageInputHandler = (input) =>
    setMessageInput(validateLength('Message', input, 6, 4000));
    ...
    <section>
      {!props.included && (
        <Head>
          ...
          <meta
            property="og:image"
            content="https://tijsvl.vercel.app/img/about__tijsvl-2000.jpg"
          />
          <meta
            name="twitter:image"
            content="https://tijsvl.vercel.app/img/about__tijsvl-2000.jpg"
          />
          <link rel="canonical" href="https://tijsvl.net/contact" />
          <meta property="og:url" content="https://tijsvl.net/contact" />
          <meta name="twitter:url" content="https://tijsvl.net/contact" />
        </Head>
      )}
      ...
      {isLoading ? (
        ...
        status === 'initial' && (
          <form
            ...
            className={classes.form + ' ' + classes.contact}
            id="contact"
          >
            <label htmlFor="name">
              Your name*{' '}
              {nameInput.error && (
                <span className={nameInput.message ? classes.red : ''}>
                  {nameInput.message}
                </span>
              )}
            </label>
            ...
            <label htmlFor="userEmail">
              Your email*{' '}
              {emailInput.error && (
                <span className={emailInput.message ? classes.red : ''}>
                  {emailInput.message}
                </span>
              )}
            </label>
            ...
            <label htmlFor="subject">
              Subject{' '}
              {subjectInput.error && (
                <span className={subjectInput.message ? classes.red : ''}>
                  {subjectInput.message}
                </span>
              )}
            </label>
            <input
            ...
            <label htmlFor="message">
              Your message*{' '}
              {messageInput.error && (
                <span className={messageInput.message ? classes.red : ''}>
      ...
      {error && (
        <div className={classes.error}>
          ...
          <p>
            ...
            <a href="mailto:info@tijsvl.net">info@tijsvl.net</a>. Your message:
          </p>
